...
anova(hs.svl.reg)
anova(hs.svl,hs.svl.reg)  #we're comparing models 
...
#Remember you can compare un-nested models with LRT, but there is more outcomes.
#2: AIC comparisons
...
#AIC is not scaled so there is no expected value. Sample size can change AIC or number of parameters
#in-class code, generationg random x varible
length(hs)
xr <- rnorm(length(hs))

fit.rand <- lm(hs~xr)
aic.summary<-AIC(hs.svl,hs.sex,hs.reg,hs.svl.reg,hs.svl.by.reg,hs.svl.sex,hs.svl.by.sex,
                 hs.ancova,hs.full,fit.rand)
#Model Averaging
c1<-exp(-.5*0)
...
#reconsider the rule of thum of AIC >4 is for multi-variate data model comparison
