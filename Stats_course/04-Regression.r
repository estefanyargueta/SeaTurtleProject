...
#evaluate models
anova(fit)
anova(fit2,fit)  #In this case, adding polynomial terms NOT an improvement
anova(fit3,fit2)
anova(fit4,fit3)
...
plot(model.anc3, type = "regression", reg.type = "RegScore", 
     predictor = X2, pch=19, col = colo)  #NOTE: plotting from RRPP a bit redundant here. 
     #It is most useful with multivariate data
abline(model.ancova$coefficients[1],model.ancova$coefficients[2])
abline((model.ancova$coefficients[1]+model.ancova$coefficients[3]),model.ancova$coefficients[2])
abline((model.ancova$coefficients[1]+model.ancova$coefficients[4]),model.ancova$coefficients[2])
abline((model.ancova$coefficients[1]+model.ancova$coefficients[5]),model.ancova$coefficients[2])
...
summary(pairwise(model.anc3, groups = SexBySurv)) 

#Pairwise comparisons of SLOPES (NOTE: makes sense only when interaction term significant)
PW <- pairwise(model.anc3, groups = SexBySurv, covariate = X2)
summary(PW, test.type = "dist")  #comparison of vector lengths
